    ...
    <div style={{display:"flex",alignItems:"center",marginBottom:22}}>
      {Array.from({length:total},(_,i)=>(
        <div key={i} style={{display:"flex",alignItems:"center",flex:i<total-1?1:0}}>
          <div className={`pg-step-dot ${i<step?"done":i===step?(isTech?"act-t":"act-u"):""}`}>{i<step?"✓":i+1}</div>
        ...
        <div className="pg-card">
          ...
          {role==="tech"&&step!==99&&(
            <>
              ...
              {/* Tech Step 1 — profile, education, achievements */}
              {step===1&&(
                <div className="pg-step-panel">
                  ...
                  <div className="pg-field tf">
                    <span className="pg-field-top">📝</span>
                    <textarea className={`pg-textarea tf${errors.bio?" err":""}`} placeholder="Write a short bio for your public profile — patients see this when choosing you..." value={tF.bio} onChange={e=>{setT("bio",e.target.value.slice(0,300));clr("bio");}} style={{paddingLeft:48,minHeight:100}}/>
              ...
              {/* Tech Step 2 — availability, radius, terms */}
              {step===2&&(
                <div className="pg-step-panel">
          ...
          {role==="user"&&step===99&&(
            <div className="pg-step-panel" style={{textAlign:"center"}}>
              <div style={{width:100,height:100,borderRadius:"50%",background:"rgba(11,94,215,.07)",border:"2px solid #0B5ED7",display:"flex",alignItems:"center",justifyContent:"center",fontSize:40,color:"#0B5ED7",animation:"pg-successP .5s ease",margin:"0 auto 22px",position:"relative"}}>
                ✓<div style={{position:"absolute",inset:-12,borderRadius:"50%",border:"1px solid rgba(11,94,215,.15)",animation:"pg-float 3.2s ease-in-out infinite"}}/>
              </div>
              ...
              <button className="pg-btn" style={{marginBottom:12}}>🔬 Browse Technicians →</button>
          ...
          {role==="tech"&&step===99&&(
            <div className="pg-step-panel" style={{textAlign:"center"}}>
              <div style={{width:100,height:100,borderRadius:"50%",background:"rgba(32,201,151,.08)",border:"2px solid #20C997",display:"flex",alignItems:"center",justifyContent:"center",fontSize:40,color:"#20C997",animation:"pg-successP .5s ease",margin:"0 auto 22px",position:"relative"}}>
                ✓<div style={{position:"absolute",inset:-12,borderRadius:"50%",border:"1px solid rgba(32,201,151,.18)",animation:"pg-float 3.2s ease-in-out infinite"}}/>
              </div>
              <div className="pg-syne" style={{fontSize:24,fontWeight:800,color:"#212529",marginBottom:6}}>Profile Live, {tF.firstName}! 🎉</div>
              ...
              <div style={{background:"rgba(32,201,151,.04)",border:"1.5px solid rgba(32,201,151,.18)",borderRadius:14,padding:"18px",marginBottom:20,textAlign:"left"}}>
                <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:12}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:"#8fa3b8"}}>{tF.specialization} · {tF.degree}</div>
                    <div style={{fontSize:11,color:"#17a680",marginTop:2}}>🟢 Accepting bookings · 📍 {tF.serviceRadius||"All areas"}</div>
                  </div>
                </div>
                {tF.bio&&<p style={{fontSize:13,color:"#495057",lineHeight:1.6,borderTop:"1px solid rgba(32,201,151,.1)",paddingTop:12,marginBottom:10}}>{tF.bio.slice(0,140)}{tF.bio.length>140?"…":""}</p>}
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {[...tF.achievements.slice(0,2),...tF.certifications.slice(0,2)].map((t,i)=>(
                    <span key={i} style={{fontSize:10,padding:"3px 10px",borderRadius:20,background:"rgba(11,94,215,.07)",color:"#0B5ED7",border:"1px solid rgba(11,94,215,.14)"}}>🏅 {t}</span>
                  ))}
                  {tF.languages.map((l,i)=>(
                    <span key={i} style={{fontSize:10,padding:"3px 10px",borderRadius:20,background:"rgba(32,201,151,.08)",color:"#17a680",border:"1px solid rgba(32,201,151,.2)"}}>🌐 {l}</span>
                  ))}
                </div>
              </div>

              <button className="pg-btn tb" style={{marginBottom:12}}>📅 Go to Dashboard →</button>
